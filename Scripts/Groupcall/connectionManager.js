...
WebRtcDemo.ConnectionManager = (function () {
    ...
    // Initialize the ConnectionManager with a signaler and callbacks to handle events
        _initialize = function (signaler, grpp, onReadyForStream, onStreamAdded, onStreamRemoved) {
            _signaler = signaler;
            grp = grpp;
        ...
        _createConnection = function (partnerClientId) {
            ...
            connection.onicecandidate = function (event) {
                if (event.candidate) {
                    ...
                    _signaler.sendSignal(JSON.stringify({ "candidate": event.candidate }),grp);
                } else {
    ...
    // Process a newly received SDP signal
        _receivedSdpSignal = function (connection, partnerClientId,grpname,sdp) {
                ...
                if (connection.remoteDescription.type == "offer") {
                    ...
                    connection.createAnswer(function (desc) {
                        connection.setLocalDescription(desc, function () {
                            _signaler.sendSignal(JSON.stringify({ "sdp": connection.localDescription }), grpname);
                        });
                    });
    ...
    // Hand off a new signal from the signaler to the connection
        _newSignal = function (cid, data) {
            var signal = JSON.parse(data),
                connection = _getConnection(cid);
            
            console.log('WebRTC: received signal from : ' + cid);

            // Route signal based on type
            if (signal.sdp) {
                _receivedSdpSignal(connection, cid, grp,signal.sdp);
                }
//            } else if (signal.candidate) {
//                _receivedCandidateSignal(connection, cid, grp,signal.candidate);
//            }
        },

        // Process a newly received Candidate signal
        _receivedCandidateSignal = function (connection, cid, grp,candidate) {
    ...
    // Send an offer for audio/video
        _initiateOffer = function (partnerClientId, grpp, stream) {
            ...
            connection.createOffer(function (desc) {
                connection.setLocalDescription(desc, function () {
                    _signaler.sendSignal(JSON.stringify({ "sdp": connection.localDescription }),grpp);
                });
            });
